// km_commands/sudo.js — مدمج من KnightBot-Mini، مُعَرَب + حقوق موحدة
...
module.exports = {
  name: 'sudo',
  category: 'owner',
  description: 'إدارة Sudo',
  ownerOnly: true,
  async execute(sock, msg, args, extra) {
    const db = extra && extra.db ? extra.db : bindDb((sock && sock._faresPhone) || '');
/**
 * Sudo Command
 * المالك can add/remove sudo users (users with owner-like privileges)
    ...
    try {
      ...
      if (action === 'remove' || action === 'rem' || action === 'del' || action === 'r') {
        ...
        if (ownerJids.some((oj) => oj.split('@')[0] === inputUser)) {
          return extra.reply('❌ المالك cannot be removed from sudo.');
        }
